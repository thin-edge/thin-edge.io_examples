...
# TODO: remove patch thin below
# pickup latest change from local repo
pushd /home/pi/workspace/selfupdate-poc/alpine/thin-edge.io_selfup >/dev/null
git diff --staged > /tmp/repo-latest-changes.patch
popd >/dev/null
mv  /tmp/repo-latest-changes.patch ./tmp/

# build images
echo "**** Building tegde docker image"
docker build -t tedge_image:v0.6.1-before-update .
echo "**** Building a 2nd image that to be used for update"
echo "**** NOTE: Same image as 1st but with different content in text file '/version'"
docker build -t tedge_image:v0.6.1-after-update -f Dockerfile-new-version-dummy .
