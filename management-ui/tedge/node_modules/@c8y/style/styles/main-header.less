// APP MAIN HEADER
.app-main-header {
  position: fixed;
  z-index: @zindex-navbar-fixed + 1;
  top: 0;
  right: 0;
  left: 0;
  max-width: 100vw;
  transition: left @open-menu-time-type, right @open-menu-time-type;

  // navigator open
  &.open {
    left: @navigatorWidth;
    .app-view {
      .c8y-app-icon,
      .app-noicon,
      .txt {
        width: 0 !important;
        margin-left: 0;
        margin-right: 0;
        .scale(0);
      }
    }
    .navigator-toggle > [class^='dlt-c8y-icon-'],
    .navigator-toggle > [class*=' dlt-c8y-icon-'],
    .navigator-toggle > .c8y-icon {
      //.rotate(180deg);
      width: 1em;
      text-align: center;
    }
  }
  // right drawer open
  &.drawerOpen {
    right: @navigatorWidth;
    .drawer-toggle > [class^='dlt-c8y-icon-'],
    .drawer-toggle > [class*=' dlt-c8y-icon-'],
    .drawer-toggle > .c8y-icon {
      //.rotate(180deg);
      width: 1em;
      text-align: center;
    }
  }

  .c8y-dropdown + .dropdown-menu > li a:not(.btn),
  .c8y-dropdown + .dropdown-menu > .dropdown-header,
  .c8y-dropdown + .dropdown-menu > li button:not(.btn) {
    //background-color: @headerColor-css-var;
    //color: @header-text-color-css-var;
    &:hover {
      //color: @header-text-color-hover-css-var;
    }
  }
  .dropdown-menu:not(.app-switcher-dropdown-menu):not(.search-mobile-menu) {
    //max-width: 240px;
    a,
    .btn {
      .text-truncate();
    }
  }
  // shadows on dropdowns inside app-main-header
  .dropdown.open .c8y-dropdown + .dropdown-menu,
  [dropdown].open .c8y-dropdown + .dropdown-menu,
  .btn-group.open .c8y-dropdown + .dropdown-menu {
    .boxShadowHelper(6, right);
  }
  .app-switcher {
    //background-color: @headerColor-css-var;
    .appLink {
      color: @component-color;
      .c8y-icon.c8y-icon-duocolor:before {
        //color: currentColor;
      }
    }
  }
}

.head-toggler {
  height: 0;
  overflow: visible;
  button {
    margin: 0 auto;
    width: 48px;
    height: 24px;
    color: @header-text-color-css-var;
    background-color: @headerColor-css-var;
    display: block;
    transition: margin 0.5s ease 0.35s;
    border: 0;
    border-radius: 0 0 24px 24px;
    .boxShadowHelper(1);
    &:focus {
      outline: none;
    }
    &:hover {
      color: @header-text-color-hover-css-var;
    }
    [class^='dlt-c8y-icon-'],
    [class*=' dlt-c8y-icon-'] {
      transition: transform 0.5s ease 0.35s;
      transform: rotate(90deg);
    }
    .head-open & {
      [class^='dlt-c8y-icon-'],
      [class*=' dlt-c8y-icon-'] {
        transform: rotate(-90deg);
      }
    }
    @media (min-width: @screen-sm-min) {
      display: none;
    }
  }
}

//header bar
.header-bar {
  top: 0;
  right: 0;
  color: @header-text-color-css-var;
  background-color: @headerColor-css-var;
  display: flex;
  flex-flow: row wrap;
  align-items: center;
  box-shadow: inset 0 -1px 0 @header-border-color;
  //.boxShadowHelper(1);

  @media (min-width: @screen-sm-min) {
    min-height: @header-bar-height;
  }
  // app icon
  .c8y-app-icon {
    font-size: 30px;
    display: inline-block;
  }
  .app-noicon {
    display: inline-block;
    &:empty {
      display: none;
    }
  }
  .c8y-app-icon,
  .app-noicon,
  .txt {
    transition: all @open-menu-time-type;
    width: 32px;
    margin-left: @margin-base;
    margin-right: @margin-base;
    .scale(1);
  }

  .main-header-button {
    &,
    & > button {
      text-decoration: none;
      background-color: transparent;
      border: 0;
      margin: 0;
      padding: 5px 15px;
      min-height: 46px;
      color: @header-text-color-css-var;
      box-shadow: inset 1px 0 0 @header-border-color;
      &:hover,
      &:focus {
        color: @header-text-color-hover-css-var;
        outline: none;
      }
      > button {
        padding: 0;
        min-height: auto;
        box-shadow: none;
      }
    }
  }


  .navigator-toggle,
  .drawer-toggle {
    font-size: 16px;
    padding-left: 8px;
    padding-right: 8px;
    > [class^='dlt-c8y-icon-'],
    > [class*=' dlt-c8y-icon-'],
    > .c8y-icon {
      transition: transform @open-menu-time-type;
    }
  }

  @media (max-width: @screen-xs-max) {
    max-height: 46px;
    overflow: hidden;
    transition: max-height 0.35s ease;
    .head-open & {
      max-height: 92px;
      overflow: visible;
    }
  }
}
// end .header-bar

// App view -
.app-view {
  display: flex;
  flex-flow: row nowrap;
  margin-right: auto;
  flex: 3 1 auto;
  border-left: 1px solid @header-border-color;
  align-items: center;
  align-self: stretch;
  width: calc(~'100% - 92px');
  max-height: 46px;
  .c8y-icon:before,
  .c8y-icon.c8y-icon-duocolor:before {
    color: currentColor;
  }
  .spinner {
    position: relative;
    margin: 0 0 0 @margin-base;
  }
}

// page header - contains the title and the breadcrumbs
.page-header {
  padding: 0 5px 0 0;
  max-height: 46px;
  width: 100%;
  max-width: calc(~'100% - 50px');
  .c8y-ui-title {
    height: 25px;
    position: relative;
    z-index: 11;
    h1 {
      display: inline-block;
      margin: 0;
      width: 100%;
      letter-spacing: -0.03em;
      font-weight: 200;
      font-size: 18px;
    }
    .c8y-ui-title-extra-info {
      position: absolute;
      top: calc(~'100% - 6px');
      right: 0;
      z-index: 12;
    }

    .edit-in-place {
      c8y-error-feedback,
      c8y-messages {
        display: none;
      }
    }
  }
}

// main-header breadcrumbs
.app-breadcrumbs {
  display: block;
  margin-top: -3px;
  max-height: 14px;
  position: relative;
  z-index: 10;
}

// main header search component
.search-header-menu {
  position: fixed;
  left: 0;
  right: 0;
  padding: 0;
  top: 92px;
  .boxShadowHelper(6, right);

  .c8y-search-form .c8y-child-assets-selector{
    .input-group{
      padding: @margin-md @margin-md 0;
      background-color: @component-background;
      .input-group-btn{
        top: @margin-base;
        right: @margin-md;
      }
    }
  }

  c8y-typeahead {
    .c8y-child-assets-selector.dropdown .dropdown-menu{
      position: relative;
      width: 100%;
      top:0;
      left:0;
      right: 0;
      padding-top: 0;
      max-height: calc(100vh - 240px);
      height: auto;
      display: block;
      box-shadow: none;
    }
    .input-group-dropdown{
      .form-control{
        border-radius: 0!important;
        border: 0;
        height: auto;
        
        padding: 16px 54px 16px 16px;
        background-color: @headerColor-css-var;
        color: @header-text-color-css-var;
        font-size: @font-size-large;
        max-height: unset !important;
        box-shadow: inset 0 0 0 4px  white, inset 0 0 0 5px @input-border;
        &:focus{
          box-shadow: inset 0 0 0 4px  white, inset 0 0 0 6px @input-border-focus;
        }
      }
    }
    .input-group-btn {
      height: 100% !important;
      .btn {
        font-size: 20px;
        height: auto !important;
        padding: 14px 16px;
        color: @header-text-color-css-var;
        &:hover {
          color: @header-text-color-hover-css-var;
        }
      }
    }
    c8y-load-more{
      padding: 0 16px;
      > .c8y-list__item{
        box-shadow: none;
      }

    }
  }
  // .input-group.input-group-search .form-control {
  //   border-radius: 0 !important;
  //   border: 0;
  //   height: auto;
  //   padding: 16px 54px 16px 16px;
  //   background-color: @headerColor-css-var;
  //   color: @header-text-color-css-var;
  //   max-height:none;
  // }
  
}

@media (min-width: @screen-sm-min) {
  .header-bar {
    flex-flow: row nowrap;
    justify-content: flex-start;
    .main-header-button {
      min-height: @header-bar-height;
    }
  }
  .app-view {
    max-height: @header-bar-height;
    width: calc(~'100% - 180px - 172px - 32px');
  }
  .page-header {
    padding: 5px;
    max-height: @header-bar-height;
    .c8y-ui-title {
      height: 34px;
      h1 {
        font-size: 26px;
        small:not(.form-control-feedback-message) {
          color: @header-text-color-css-var;
          opacity: 0.8;
        }
      }
    }
    .text-muted {
      color: @header-text-color-css-var;
      opacity: 0.8;
    }
  }
  .search-header-menu {
    top: @header-bar-height;
    left: @margin-lg;
    right: @margin-lg;
    margin-left: auto;
    margin-right: auto;
  }
}
@media (min-width: @grid-float-breakpoint) {
  .search-header-menu {
    left: @navigatorWidth;
    right: @navigatorWidth;
    max-width: 1200px;
    .app-main-header.open & {
      left: (@navigatorWidth * 2);
    }
  }
}

// Smartphone only
@media (max-width: @screen-xs-max) {
  .header-bar {
    // order of components
    .navigator-toggle {
      order: 1;
    }
    .app-view {
      order: 2;
    }
    .drawer-toggle {
      order: 3;
    }
    .main-header-item {
      order: 5;
    }
  }
  .main-header-item {
    flex: 1 1 auto;
    .shadow-border(1px; @header-border-color);
    .main-header-button,
    .dropdown {
      width: 100%;
    }
    .dropdown-menu:not(.app-switcher-dropdown-menu) {
      min-width: 100%;
    }
  }
  .app-main-header {
    &.open,
    &.drawerOpen {
      .header-bar {
        overflow: hidden;
      }
      .app-view {
        .c8y-app-icon,
        .app-noicon,
        .txt {
          width: 0 !important;
          margin-left: 0;
          margin-right: 0;
          .scale(0);
        }
      }
      .page-header {
        display: none;
      }
    }
    // navigator and right drawer open
    &.drawerOpen.open {
      .main-header-item {
        display: none;
      }
    }
  }
}
