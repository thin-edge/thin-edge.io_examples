...
import { NavigatorNode, gettext, HOOK_NAVIGATOR_NODES, HOOK_TABS, Status, ModalService, AlertService, CoreModule, CommonModule, FormsModule, ConditionalTabsOutletComponent, Permissions, ProviderConfigurationModule } from '@c8y/ngx-components';
...
let AbstractConnectivityNodeFactory = class AbstractConnectivityNodeFactory {
...
let ActilityTabFactory = class ActilityTabFactory extends AbstractConnectivityTabFactory {
    constructor(router, guard) {
        super(router, guard);
        this.router = router;
        this.guard = guard;
...
let ActilityNodeFactory = class ActilityNodeFactory extends AbstractConnectivityNodeFactory {
    constructor(tabFactory) {
        super(tabFactory);
        this.tabFactory = tabFactory;
    }
...
let ActilityModule = class ActilityModule {
};
...
var ImpactGuard_1;
let ImpactGuard = ImpactGuard_1 = class ImpactGuard {
    constructor(applicationService) {
        this.applicationService = applicationService;
    }
    canActivate(route, state) {
...
let SigfoxTabFactory = class SigfoxTabFactory extends AbstractConnectivityTabFactory {
    constructor(router, guard) {
        super(router, guard);
        this.router = router;
        this.guard = guard;
...
let SigfoxNodeFactory = class SigfoxNodeFactory extends AbstractConnectivityNodeFactory {
    constructor(tabFactory) {
        super(tabFactory);
        this.tabFactory = tabFactory;
    }
...
let SigfoxModule = class SigfoxModule {
};
